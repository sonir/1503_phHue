...
void ofxPhilipsHue::setAllState(bool state, ofColor c, int transitionDuration){
    ...
    setAllState( state, b, s, h, transitionDuration );
}
...
bool ofxPhilipsHue::sendToAll(string json){
    ...
    float timeOut = 1.0; //seconds
    Poco::URI uri = Poco::URI( "http://" + bridge + "/api/" + apiUser +"/groups/0/action" );
    std::string path(uri.getPathAndQuery());
    if (path.empty()) path = "/";
    string host = uri.getHost();
    ...
    try{
        ...
        ofLog( OF_LOG_ERROR, "ofxPhilipsHue::sendToAll(%s) to %s >> Exception: %s\n", json.c_str(), uri.toString().c_str(), exc.displayText().c_str() );
        return false;
    }
